...
#  --CPU	Default: 4       
#  --end_len	how many base paires should be trimmed from the sequences
###################################################################################
...
&GetOptions (
			 ...
			 "CPU=i" => \$CPU,    #分配的CPU数量
             "end_len=i" => \$end_len,#需要从5’端切除多少个碱基
             );
...
sub trim_end{
	my $line=shift;	# filelist中每行是文件名+adapter	
	my @cols = split(/\t/, $line);
	my $input= $cols[0]; #提取文件名，不带后缀
	open FQIN,'<',$input.'.fastq' or die "Can't open input file $input:$!";
	our $output=$input.'.fq';
	open FQOUT,'>',$output or die "Can't open output file $output:$!";
	while(<FQIN>){
		unless($.%2){
			print FQOUT substr($_,$end_len);	# 提取序列从第$end_len+1个位置到结束
		}else{
			print FQOUT $_;
		}
	}
	...
	close(FQOUT);	
	0;	#能处理到这里，应都是正常结束
	system("mv $input.fastq $input.raw"); # 原始输入文件保存为“.fq”
	system("mv $output $input.fastq");   # 切除后文件保存为“.fastq”
}
