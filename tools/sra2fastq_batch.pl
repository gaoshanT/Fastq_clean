...
#################################################################################
# $0 --file_list <string> --rm_SRA --CPU [Int] --shuffle
#  --file_list	a file containing a list of sra file without suffix(one SRA number per line)
#  --rm_SRA	Remove sra file after dumping or not(1 for remove//0 for not) Default:0       
#  --CPU	Default: 4
...
&GetOptions (
			 ...
			 "rm_SRA!" => \$rm_SRA,    #数据文件的后缀名
			 "CPU=i" => \$CPU,    #分配的CPU数量
             "shuffle!" => \$shuffle_flag #默认不需要shuffle
             );
...
main: {
	...
	while (<$fh>) {
		...
		my @cols = split(/\t/, $_);
		my $cmd;
		if($rm_SRA){#主要就是这句与cmd_process_forker.pl不同
			$cmd="$BIN_DIR/fastq-dump $cols[0].sra; rm $cols[0].sra"; 
		}else{
			$cmd="$BIN_DIR/fastq-dump $cols[0].sra"; 
		}
